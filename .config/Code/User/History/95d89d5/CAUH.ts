...
import { images, products } from "../db/schema.js";
import { MapProductsData, ProductsModel } from "../models/products.model.js";
...
export async function getProductByIdService(id: any): Promise<ProductsModel> {
  const database = await db;
...
export async function getDealProductsService(query?: any[]): Promise<any> {
  const database = await db;
  const productsData = await database
    .select()
    .from(products)
    .where(
      inArray(
        products.product_id,
        query || [1000001, 1000002, 1000003, 1000004]
      )
    );
  ...
  const productMappedData = MapProductsData(productsData, imagesData);
